...
NameServer就是为了解决以上问题设计的。 nameserver监听broker的心跳，如果broker宕机了就删除当前对应的消费者。
...
```java
//创建NamesrvConfig，nameserver属性的配置
final NamesrvConfig namesrvConfig = new NamesrvConfig();
//创建NettyServerConfig，nettyserver的属性配置，服务端的配置
final NettyServerConfig nettyServerConfig = new NettyServerConfig();
//设置启动端口号
nettyServerConfig.setListenPort(9876);
//解析启动-c参数，如果有参数c，文件路径存在的话就将读取配置文件。
...
**serverPooledByteBufAllocatorEnable：**ByteBuffer是否开启缓存，堆外面创建缓冲区，;
...
在JVM进程关闭之前，先将线程池关闭，及时释放资源，注册的回调方法。资源的释放
...
![image-20220616194047179](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220616194047179.png)
...
```java
//检查生产者组是否满足要求，校验生产者是不是满足，组名不能是默认
this.checkConfig();
//更改当前instanceName为进程ID，进程实例的名字
...
public MQClientInstance getAndCreateMQClientInstance(final ClientConfig clientConfig, 
    ...
    //实例如果为空就创建新的实例,并添加到实例表中。拿到生产者的实例，将生产者的实例放到
    if (null == instance) {
        instance =
...
![image-20220617091120788](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220617091120788.png)
...
消息发送选择broker路由机制：发送消息的时候，先校验消息的长度，合法性，从生产者缓存中获取broker的地址，如果没有获取到broker地址就从nameserver中获取路由信息，获取路由信息返回路由信息，将路由信息更新到本地缓存中。
...
####2）查找路由，根据当前的topic来查找路由，生产者在本地缓存路由表，没有查询到就去nameserver中查找	
...
判断路由是不是需要更新，如果需要更新的话就将新就进行替换

```java
//判断路由是否需要更改
TopicRouteData old = this.topicRouteTable.get(topic);
boolean changed = topicRouteDataIsChange(old, );
if (!changed) {
...
***代码：MQClientInstance#updateTopicRouteInfoFromNameServer***更新route信息
...
* 默认不启用Broker故障延迟机制，取出一个索引，索引自增之后就可以查询出队列，规避上一次的broker队列，做到负载均衡
...
public MessageQueue selectOneMessageQueue(final String lastBrokerName) {
    ...
    if (lastBrokerName == null) {
        ...
        //sendWhichQueue，一个索引对所有的megQueue取模+1
        int index = this.sendWhichQueue.getAndIncrement();
        //遍历消息队列集合
        for (int i = 0; i < this.messageQueueList.size(); i++) {
            //sendWhichQueue自增后取模，每一次取出的值都是不一样的。
...
* 启用Broker故障延迟机制：如果开启了故障延迟，返回

```java
public MessageQueue selectOneMessageQueue(final TopicPublishInfo tpInfo, final String lastBrokerName) {
    //Broker故障延迟机制，选择轮询获取队列发送消息，
...
默认超时时间是3S，如果消息合法就开始查找路由，本地缓存路由，如果本地查出来就使用本地，如果本地没有就去nameserver查找topic信息，消息发送到消息队列中，故障延迟机制
...
*batch方法对多条的消息进行编码，校验合法性，压缩消息一起发送*
...
消息追加到内存中，使用刷盘机制将内存中的消息追加到内存中。如果是从节点就不写入。判断内存中是不是可用的，
...
//获得该消息在消息队列中的偏移量。获取消息的长度，如果大于文件的长度就
...
- commitLog：消息存储目录：消息的元数据
- config：运行期间一些配置信息
- consumerqueue：消息消费队列存储目录，消息索引信息
- index：消息索引文件存储目录，索引存储
...
####1）MappedFileQueue，文件的存储目录，文件的大小对应commitlog文件夹
...
如果message外面有共享缓存区的话，就将数据写入到buffer中，根据filechannel进行传输。进行刷盘，

![image-20220617103747126](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220617103747126.png)
...
获取当前文件最大可读指针。如果writeBuffer为空，则直接返回当前的写指针（文件的最大容量）；如果writeBuffer不为空（文件上一次存储的位置），则返回上一次提交的指针。在MappedFile设置中,只有提交了的数据（写入到MappedByteBuffer或FileChannel中的数据）才是安全的数据
...
消息消费队文件、消息属性索引文件都是基于CommitLog文件构建的，当消息生产者提交的消息存储在CommitLog文件中，ConsumerQueue、IndexFile需要及时更新，否则消息无法及时被消费，根据消息属性查找消息也会出现较大延迟。

RocketMQ通过开启一个线ReputMessageService来准实时转发CommitLog文件更新事件，相应的任务处理器根据转发的消息及时更新ConsumerQueue、IndexFile文件。

defaultMessageStore启动的时候，就会将线程创建出来，每隔1ms进行数据的分发
...
public void buildIndex(DispatchRequest req) {
    ...
    if (indexFile != null) {
        ...
        //如果该消息的物理偏移量小于索引文件中的最大物理偏移量,则说明是重复数据,忽略本次索引构建。当前的消息已经使用过
...
如果消息成功存储到CommitLog文件中，转发任务未成功执行，此时消息服务器Broker由于某个愿意宕机，导致CommitLog、ConsumerQueue、IndexFile文件数据不一致。

如果不加以人工修复的话，会有一部分消息即便在CommitLog中文件中存在，但由于没有转发到ConsumerQueue，这部分消息将永远复发被消费者消费。

![](img/文件恢复总体流程.png)

####1）存储文件加载

***代码：DefaultMessageStore#load***	commitlog数据进行同步
...
```java
//判断临时文件是否存在，如果有临时文件就是一个正常退出，没有就是错误退出
...
消息偏移量：message是无限长的queue，偏移量是文件的地址
...
消息追加到内存后，立即将数据刷写到磁盘文件，根据刷盘的策略进行数据的同步

![](img/同步刷盘流程.png)

***代码：CommitLog#handleDiskFlush***，消息拿到直接刷到磁盘

```java
//刷盘服务
final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
if (messageExt.isWaitStoreMsgOK()) {
    //封装刷盘的数据成为请求，偏移量和读取的bytes
...
private void doCommit() {
    ...
    synchronized (this.requestsRead) {
        if (!this.requestsRead.isEmpty()) {
            ...
            //更新刷盘监测点，刷盘的位置大于写的位置，代表有新添加出的数据，更新刷盘的存储点
...
2. CommitRealTimeService线程每隔200ms将ByteBuffer新追加内容提交到MappedByteBuffer中（新线程不断的将堆外内存添加到Buffer中）
...
5. FlushRealTimeService线程默认每500ms将MappedByteBuffer中新追加的内存刷写到磁盘
   ...
   追加到磁盘得线程，将新Buffer中新加的数据500ms写到磁盘中。
...
指定文件删除的条件：磁盘空间如果不充足就删除过期文件，指定删除文件的时间点，RocketMQ默认一天的四点进行删除。预留删除，手工触发
...
consumerQueue和主题和队列中的文件

indexfile：hash索引组织，

根据这两个文件就可以很快的在commitlog中找到对应的消息。

当消息达到CommitLog后，会通过ReputMessageService线程接近实时地将消息转发给消息消费队列文件与索引文件。为了安全起见，RocketMQ引入abort文件（启动创建出来，正常关闭就会删除掉），记录Broker的停机是否是正常关闭还是异常关闭，在重启Broker时为了保证

CommitLog文件，消息消费队列文件与Hash索引文件的正确性，分别采用不同策略来恢复文件。正常回复是从倒数第三个commitlog文件回复，异常恢复是从最后一个文件恢复。
...
消息消费以组的模式开展，一个消费组内可以包含多个消费者，每一个消费者组可订阅多个主题topic，消费组之间有负载均衡式和广播模式两种消费模式。集群模式，主题下的同一条消息只允许被其中一个消费者消费。广播模式，主题下的同一条消息，将被集群内的所有消费者消费一次。

消息服务器与消费者之间的消息传递也有两种模式：推模式、拉模式。所谓的拉模式，是消费端主动拉起拉消息请求，而推模式是消息达到消息服务器后，推送给消息消费者。RocketMQ消息推模式的实现基于拉模式，在拉模式上包装一层，一个拉取任务完成后开始下一个拉取任务。
...
start方法：检查消息的合法性，构建主题的信息，构建MQclient实例，负载均衡的策略，
...
广播模式比较简单，每一个消费者需要拉取订阅主题下所有队列的消息。本文重点讲解集群模式。在集群模式下，同一个消费者组内有多个消息消费者，同一个主题存在多个消费队列，消费者通过负载均衡的方式消费消息。

消息队列负载均衡，通常的作法是一个消息队列在同一个时间只允许被一个消费消费者消费，一个消息消费者可以同时消费多个消息队列。

Rocket支持顺序消息的消费，也就是保证在同一个主题上的消息队列是顺序消费的，不支持全局顺序，如果要实现全局的顺序，需要将主题和broker都是1。只有一个队列
...
***代码：PullMessageService#run	拉消息的方法使用循环来拉取***
...
将push拉取到的消息（messageQueue）转换到ProcessQueue中，messageQueue就可以不断拉取。
...
#####1.客户端发起拉取请求，先获取到processQueue，队列是不是丢弃了，
...
消息到的时候消息会进行存储，如果同一时间达到后就挂起消息，

消息拉取总结：run->拉取请求队列，拉取消息，将消息转发存储到processQueue中，获取topic和系统标记，获取broker地址，

![image-20220617161200719](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220617161200719.png)
